...
const _sfc_main = /* @__PURE__ */ common_vendor.defineComponent({
  ...
  setup(__props) {
    ...
    const formatJoinDateLabel = (value) => {
      if (!value)
        return "";
      if (typeof value === "string") {
        ...
        if (!Number.isNaN(parsed.getTime())) {
          const year2 = parsed.getFullYear();
          const month2 = `${parsed.getMonth() + 1}`.padStart(2, "0");
          const day2 = `${parsed.getDate()}`.padStart(2, "0");
    ...
    const initUserFromStorage = () => {
      try {
        ...
        common_vendor.index.__f__("log", "at pages/profile/index.vue:164", 117, "stored", stored);
        if (stored) {
        ...
        common_vendor.index.__f__("warn", "at pages/profile/index.vue:171", "读取用户信息失败", error);
        user.value = { ...defaultProfile };
      }
      refreshLoginState();
    };
    ...
    const handleLoginSuccess = (payload) => {
      ...
      common_vendor.index.__f__("log", "at pages/profile/index.vue:200", 152, "finalProfile = ", finalProfile);
